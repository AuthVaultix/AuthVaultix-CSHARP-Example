    ...
    public class api
    {
        ...
        public void register(string username, string pass, string key, string email = "")
        {
            ...
            if (json.success)
            {
                ...
                {
                    Console.WriteLine("⚠️ Login success flag true, but user info is null (parse issue).");
                }
            }
            else
            {
                Console.WriteLine("❌ Login failed: " + (json.message ?? "Error to Login!"));
            }
        }
        ...
        public static void error(string message)
        {
            ...
            {
                // logging failure ignored (VERY IMPORTANT)
            }
        ...
        private static string req(NameValueCollection post_data)
        {
            try
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    sw.Start();
                    byte[] raw_response = client.UploadValues("https://api.authvaultix.com/api/1.2/", post_data);
                    sw.Stop();
                    responseTime = sw.ElapsedMilliseconds;
                    string response = Encoding.UTF8.GetString(raw_response);
                    string signature = client.ResponseHeaders["signature"];
                    sigCheck(response, signature, post_data.Get(0));
            ...
            catch (WebException webex)
            {
                if (webex.Response is HttpWebResponse response)
                {
                    switch (response.StatusCode)
                    {
                        ...
                        default:
                            error("Connection failure. Please try again later.");
                            break;
                    }
                }
                else
                {
                    error("Network error. Check internet or firewall.");
                }
        ...
        public class user_data_class
        {
            ...
            public DateTime? CreationDate
            {
                get
                {
                    if (string.IsNullOrWhiteSpace(createdate) || createdate == "NULL")
                        return null;

                    if (!long.TryParse(createdate, out long ts) || ts <= 0)
                        return null;

                    return AuthVaultix.api.UnixTimeToDateTime(ts);
                }
            }
            ...
            public DateTime? LastLoginDate
            {
                get
                {
                    if (string.IsNullOrWhiteSpace(lastlogin) || lastlogin == "NULL")
                        return null;

                    if (!long.TryParse(lastlogin, out long ts) || ts <= 0)
                        return null;
